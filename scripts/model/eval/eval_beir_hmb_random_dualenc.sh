#!/bin/bash -i
#SBATCH --job-name=eval_hmb_random
#SBATCH --output=logs/slurm/eval_hmb_random_%j.out
#SBATCH --error=logs/slurm/eval_hmb_random_%j.err
...
MODEL="outputs/models/dual_encoder/cls_pooling/beir_hebrew_hn/hebmodernbert/HebrewModernBERT-base-random/model"
LABEL="hebrewmodernbert-base-random"
BATCH_SIZE=256
MAX_LENGTH=512
RESULTS_DIR="outputs/eval/beir_zeroshot"

echo "Model: $MODEL (RANDOM WEIGHTS BASELINE — label: $LABEL)"
...
python src/model/eval/collect_beir_results.py \
    ...
    --output_csv "${RESULTS_DIR}/summary_hmb_random.csv"
