#!/bin/bash -i
#SBATCH --job-name=eval_ndb_hn_dualenc
#SBATCH --output=logs/slurm/eval_ndb_hn_dualenc_%j.out
#SBATCH --error=logs/slurm/eval_ndb_hn_dualenc_%j.err
...
MODEL="outputs/models/dual_encoder/cls_pooling/beir_hebrew_hn/neodictabert/dicta-il_NeoDictaBERT/model"
LABEL="neodictabert-dualenc-beir-hn"
BATCH_SIZE=256
MAX_LENGTH=512
...
python src/model/eval/collect_beir_results.py \
    ...
    --output_csv "${RESULTS_DIR}/summary_ndb_hn.csv"
